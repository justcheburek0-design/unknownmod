...
Примечание: если раскрытый игрок выходит, его 15-минутный таймер ставится на паузу и продолжится после входа. Следующее событие `reveal` через 2 часа при этом не сдвигается. Несколько раскрытых игроков одновременно теперь поддерживаются.
