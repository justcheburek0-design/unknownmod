...
Серверный Fabric-мод для режима социальной дедукции. Он скрывает личности игроков, переводит их в `ghost`-состояние, запускает `reveal` и отправляет кастомные сообщения о важных событиях.
...
Часть библиотек уже лежит в `libs/`, но Fabric, Polymer Core и SkinRestorer должны быть доступны на сервере.
...
4. Проверьте, что рядом установлены Fabric API, Polymer Core и SkinRestorer.
...
- `interval-hours` и `duration-minutes` не должны быть меньше `1`.
...
| Команда | Что делает |
| --- | --- |
| `/unknown reload` | Перечитывает `config/unknown/config.yml` и применяет изменения без перезапуска. |
...
| `/unknown reveal status` | Показывает текущий статус таймера или активных `reveal`. |

Если раскрытый игрок выходит, его 15-минутный таймер ставится на паузу и продолжится после входа. Следующее событие `reveal` через 2 часа при этом не сдвигается. Несколько раскрытых игроков одновременно тоже поддерживаются.
...
- Если нужно вернуть отдельный блок обратно в обработку, добавь `allow:` в начале, например `allow:grass_block`.
